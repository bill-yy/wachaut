import { d as sanitize_props, f as spread_props, c as slot, h as head, s as store_get, e as escape_html, a as attr, u as unsubscribe_stores, p as pop, b as push } from "../../../chunks/index.js";
...
function _page($$payload, $$props) {
  ...
  if (store_get($$store_subs ??= {}, "$isLoading", isLoading)) {
    $$payload.out += "<!--[-->";
    $$payload.out += `<main class="flex min-h-screen flex-col items-center justify-center bg-slate-50"><div class="mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-slate-100"><div class="h-8 w-8 animate-spin rounded-full border-4 border-slate-300 border-t-slate-600"></div></div> <p class="text-lg font-semibold text-slate-700">Preparando sala...</p> <p class="mt-2 text-sm text-slate-400">Un momento</p></main>`;
  } else {
    $$payload.out += "<!--[!-->";
    ...
    if (store_get($$store_subs ??= {}, "$error", error)) {
      $$payload.out += "<!--[-->";
      $$payload.out += `<div class="mx-4 mt-4 animate-slide-up"><div class="mx-auto flex max-w-6xl items-center gap-3 rounded-xl bg-red-50 px-4 py-3 border border-red-100">`;
    ...
    if (store_get($$store_subs ??= {}, "$isSharing", isSharing)) {
      ...
      $$payload.out += `<!--]--></button> <button class="flex h-10 w-10 items-center justify-center rounded-xl bg-white/20 text-white backdrop-blur-sm transition-colors hover:bg-white/30"${attr("title", store_get($$store_subs ??= {}, "$isFullscreen", isFullscreen) ? "Salir de pantalla completa" : "Pantalla completa")}>`;
      ...
      $$payload.out += `<!--]--></button> <button class="flex h-10 items-center justify-center gap-2 rounded-xl bg-red-500/90 px-4 text-sm font-medium text-white backdrop-blur-sm transition-colors hover:bg-red-600" title="Detener compartir">`;
      Circle_stop($$payload, { class: "h-5 w-5" });
      $$payload.out += `<!----> <span class="hidden sm:inline">Detener</span></button></div>`;
    } else {
      $$payload.out += "<!--[!-->";
      $$payload.out += `<div class="flex h-full flex-col items-center justify-center text-white"><div class="mb-6 flex h-20 w-20 animate-float items-center justify-center rounded-2xl bg-slate-700/50">`;
      Monitor($$payload, { class: "h-10 w-10 text-slate-400" });
      $$payload.out += `<!----></div> <p class="mb-2 text-xl font-semibold text-slate-300">Tu pantalla aparecerá aquí</p> <p class="mb-6 text-sm text-slate-500">Haz clic para empezar a compartir</p> <button class="btn-primary gap-2 px-6 py-3.5">`;
    ...
    if (store_get($$store_subs ??= {}, "$copiedPin", copiedPin)) {
      $$payload.out += "<!--[-->";
      $$payload.out += `<p class="mt-1.5 text-xs text-green-600 animate-fade-in">¡PIN copiado!</p>`;
    } else {
      $$payload.out += "<!--[!-->";
    }
    ...
    if (store_get($$store_subs ??= {}, "$copiedUrl", copiedUrl)) {
      $$payload.out += "<!--[-->";
      $$payload.out += `<p class="mt-1.5 text-xs text-green-600 animate-fade-in">¡Enlace copiado!</p>`;
    } else {
      $$payload.out += "<!--[!-->";
    }
    ...
    $$payload.out += `<!--]--> <button class="btn-secondary w-full py-3.5">Cerrar sala</button></div></div> <div class="rounded-xl bg-blue-50 p-4 border border-blue-100"><div class="flex items-start gap-3"><div class="mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-blue-200 text-xs font-bold text-blue-700">i</div> <div><h4 class="mb-1 text-xs font-bold text-blue-800">Consejo</h4> <p class="text-xs leading-relaxed text-blue-600">Comparte el PIN o el enlace con tus amigos para que se unan a la sala. Máximo 5 espectadores.</p></div></div></div></div></div></div></main> `;
    if (store_get($$store_subs ??= {}, "$showLeaveConfirm", showLeaveConfirm)) {
      $$payload.out += "<!--[-->";
      $$payload.out += `<div class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in"><div class="mx-4 w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl animate-scale-in"><div class="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-amber-50">`;
      Triangle_alert($$payload, { class: "h-6 w-6 text-amber-500" });
      $$payload.out += `<!----></div> <h3 class="mb-2 text-lg font-bold text-slate-800">¿Cerrar la sala?</h3> <p class="mb-6 text-sm text-slate-500">`;
      if (store_get($$store_subs ??= {}, "$isSharing", isSharing)) {
        $$payload.out += "<!--[-->";
        $$payload.out += `Se detendrá la transmisión y los espectadores perderán la conexión.`;
      } else {
        $$payload.out += "<!--[!-->";
        $$payload.out += `Los espectadores perderán la conexión.`;
      }
      $$payload.out += `<!--]--></p> <div class="flex gap-3"><button class="btn-secondary flex-1 py-3">Cancelar</button> <button class="btn-danger flex-1 py-3">Cerrar sala</button></div></div></div>`;
    } else {
      $$payload.out += "<!--[!-->";
    }
    $$payload.out += `<!--]-->`;
