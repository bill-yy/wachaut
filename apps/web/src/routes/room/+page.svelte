...
<svelte:head>
	<title>{isSharing ? 'Compartiendo pantalla - Wachaut' : 'Sala de Wachaut'}</title>
</svelte:head>
	...
	<header class="sticky top-0 z-50 border-b border-slate-200/80 bg-white/80 px-4 py-3 backdrop-blur-xl">
		<div class="mx-auto flex max-w-6xl items-center justify-between">
			<div class="flex items-center gap-3">
				<button onclick={goHome} class="flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-700" title="Volver al inicio">
			...
			<div class="relative mx-auto aspect-video max-w-5xl overflow-hidden rounded-2xl bg-slate-800 shadow-2xl">
				{#if isSharing}
					...
					<div class="absolute bottom-0 left-0 right-0 flex items-center justify-center gap-2 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-5 opacity-0 transition-opacity duration-300 hover:opacity-100">
						...
						<button onclick={stopSharing} class="flex h-10 items-center justify-center gap-2 rounded-xl bg-red-500/90 px-4 text-sm font-medium text-white backdrop-blur-sm transition-colors hover:bg-red-600" title="Detener compartir">
				...
				{:else}
					<div class="flex h-full flex-col items-center justify-center text-white">
						<div class="mb-6 flex h-20 w-20 animate-float items-center justify-center rounded-2xl bg-slate-700/50">
							<Monitor class="h-10 w-10 text-slate-400" />
						</div>
			...
			<div class="space-y-5">
				...
				<div class="card-static">
					<div class="mb-4 flex items-center gap-2">
						...
						<h3 class="text-sm font-bold text-slate-800">Información de la sala</h3>
					</div>
				...
				<div class="card-static">
					...
					<div class="space-y-2.5">
						...
						{:else}
							<button onclick={stopSharing} class="btn-danger w-full gap-2 py-3.5">
								<StopCircle class="h-4 w-4" />
								Detener compartir
							</button>
						{/if}
						<button onclick={openLeaveConfirm} class="btn-secondary w-full py-3.5">
							Cerrar sala
						</button>
					</div>
				</div>
				...
				<div class="rounded-xl bg-blue-50 p-4 border border-blue-100">
					<div class="flex items-start gap-3">
						<div class="mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-blue-200 text-xs font-bold text-blue-700">i</div>
						<div>
							<h4 class="mb-1 text-xs font-bold text-blue-800">Consejo</h4>
							<p class="text-xs leading-relaxed text-blue-600">
								Comparte el PIN o el enlace con tus amigos para que se unan a la sala. Máximo 5 espectadores.
							</p>
						</div>
					</div>
				</div>
	...
	<div class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in">
		<div class="mx-4 w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl animate-scale-in">
			...
			<div class="flex gap-3">
				<button onclick={cancelLeave} class="btn-secondary flex-1 py-3">
					Cancelar
				</button>
				<button onclick={confirmLeave} class="btn-danger flex-1 py-3">
					Cerrar sala
				</button>
			</div>
		</div>
