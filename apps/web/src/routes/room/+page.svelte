<script lang="ts">
	...
	function toggleMute() {
		if (stream) { const t = stream.getAudioTracks()[0]; if (t) { t.enabled = !t.enabled; isMuted = !t.enabled; } }
	}
	...
	async function copyToClipboard(text: string, type: 'pin' | 'url') {
		...
		else { copiedUrl = true; setTimeout(() => copiedUrl = false, 2000); }
	}

	function goHome() {
		if (isSharing || viewerCount > 0) showLeaveConfirm = true;
		else confirmLeave();
	}
...
<div class="fixed inset-0 z-[200] flex items-center justify-center bg-slate-50 transition-opacity duration-500" class:opacity-0={isReady} class:pointer-events-none={isReady}>
	<div class="text-center">
		...
		<p class="text-sm text-slate-500">Preparando sala...</p>
	</div>
</div>
...
<main class="min-h-screen flex-col bg-slate-50 transition-opacity duration-500" class:flex={isReady} class:hidden={!isReady} class:opacity-0={!isReady}>
	...
	{#if errorMsg}
		<div class="mx-4 mt-4"><div class="mx-auto flex max-w-6xl items-center gap-3 rounded-xl border border-red-100 bg-red-50 px-4 py-3">
			<AlertTriangle class="h-4 w-4 flex-shrink-0 text-red-500" /><p class="text-sm text-red-600">{errorMsg}</p>
		</div></div>
	{/if}
			...
			<div class="space-y-5">
				<div class="card-static">
					...
					<div class="mb-4">
						...
						<div class="flex items-center gap-2">
							<code class="flex-1 rounded-xl bg-slate-100 px-4 py-3 text-center font-mono text-xl font-bold tracking-[0.3em] text-slate-800">{pin}</code>
	...
	<div class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
		<div class="mx-4 w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
			...
			<div class="flex gap-3">
				<button onclick={() => showLeaveConfirm = false} class="btn-secondary flex-1 py-3">Cancelar</button>
